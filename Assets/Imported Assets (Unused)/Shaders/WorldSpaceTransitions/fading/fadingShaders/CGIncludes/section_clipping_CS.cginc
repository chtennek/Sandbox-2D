...
//Plane clipping definitions
	...
	//static const float _atlasSize = 8;//pow(_NoiseAtlas_TexelSize.z, 1/6);
	//static const float _atlasTileSize = pow(_NoiseAtlas_TexelSize.z, 1/3);
	...
	float texAtlas(float3 co, sampler2D _noise, float _noiseScale)
	 {
		 ...
		 float3 col = tex2D(_noise, float2(x1,y1));
	...
	#define PLANE_CLIPWITHCAPS(posWorld) PlaneClipWithCaps(posWorld); //preprocessor macro that will produce an empty block if no clipping planes are used.
		...
		inline float4 fadeTransition(float3 posWorld)
		{
			...
			float4 rgbcol = float4(0,0,0,0);
				...
				rgbcol = tex2D(_Curves, float2(transparency,0.5));
				float4 col = tex2D(_Curves, float2(transparency,1));
				transparency = col.r;
			#endif

			#if DISSOLVE_GLOW
				 transparency = (1-2*_inverse)*(transparency - texAtlas(posWorld, _NoiseAtlas, _Noise3dScale));
			#endif
